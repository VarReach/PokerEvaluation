import { Card, convertCard, convertCardRankToName } from './PokerCard';

export default function evaluatePokerHand(handString: string): string {
  let hand: any[] = handString.split(' ');
...
function validatePokerHand(hand: string[]): void {
  if (hand.length !== 5) {
...
function sortPokerHand(a: Card, b: Card): number {
  if (a.rank < b.rank) {
    return -1;
...
function checkForFlush(hand: Card[]): boolean {
...
function checkForStraight(hand: Card[]): boolean {
  for (let i = 1; i < hand.length; i++) {
...
function getUniqueRankCounts(hand: Card[]) {
  const count = {};
  hand.forEach(card => {
    count[card.rank]
      ? count[card.rank] += 1
